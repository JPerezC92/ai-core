...
- ✅ Migration establishes the destination project as the active identity in an `adapted` root runtime and records AICore separately as upstream provenance, including ancestor and destination-local version markers. Evidence: `.opencode/skills/migrate-core-to-project/SKILL.md` 2.1.0 step 1 (confirmed destination identity), step 5 (`Project identity` + `Upstream provenance` + ancestor spec version + destination `Local version: 1.0.0` + destination-facing lineage), step 6.1 (`root-runtime-spec` mode `adapted`); Vault 🔐 (Catalog Steward) `[PASS]`.
