...
- ✅ One machine catalog `.aicore/core-catalog-v2.yaml` defines unit identity, logical members, machine `applicability` (`always`/`requires`/`any_of`), projections (`file`/`guarded_file`/`tree`/`assertions`), and the deterministic config assertions; both management tools and `verify-all` consume it. Evidence: `.aicore/core-catalog-v2.yaml` (35 units, 2 assertion units); `sync_aicore_adoption.py` `load_catalog`.
...
- ✅ Migration (`migrate-core-to-project` 2.2.0) bootstraps a complete applicable-set enrollment and delegates lock generation and verification to the sync engine; there is no selective or deferred copy. Evidence: `.opencode/skills/migrate-core-to-project/SKILL.md` 2.2.0; no `Select items`/`deferred` language remains.
...
- ✅ The v2 suite is stdlib `unittest` with no new dependency or CI workflow, and the engine passes the Python/script architecture gate. Evidence: 45 tests OK; Bastion 🧱 (Backend & Scripts Architect) `[PASS]`; Crucible 🔥 (Test Architect) `[PASS]`; `pyproject.toml`/`uv.lock` unchanged.
...
- ✅ Adopter root runtimes identify only the destination project and contain no AICore identity, repository, management-tool, reuse-guide, provenance, or lineage references; source provenance remains required in `.aicore` controls. A closed `guarded_file` projection with the `adopter_root_runtime` destination policy enforces the boundary: `propose-lock` fails with stable code `policy_violation` and emits no YAML, and `check` reports the disposition `policy_violation` and exits 1 even when the locked bytes already match. Evidence: `sync_aicore_adoption.py` `_adopter_root_policy_violation`/`_destination_policy_violation`; catalog `guarded_file`/`adopter_root_runtime`; 45-test suite incl. clean-root, missing-marker, original reuse-guide, verbose-lineage, minimal-lineage, and locked-violation cases; Bastion 🧱 (Backend & Scripts Architect) `[PASS]`, Crucible 🔥 (Test Architect) `[PASS]`, Vault 🔐 (Catalog Steward) `[PASS]`, Sentinel 🛡️ (Quality Guardian) `[PASS]`.
- ✅ Shared dependency-audit instructions derive project identity, dependency expectations, framework conventions, and supported package-manager branches from each active repository rather than carrying AICore-only assumptions into adopters. Evidence: `.opencode/agents/warden.md` 1.2.0 manifest/lock-pair discovery with JavaScript, Python, no-root-dependency, and manifest-without-lock branches; repository-derived identity/dependencies; conditional framework prefix; Sentinel 🛡️ (Quality Guardian) `[PASS]`.

## Change log

- 2026-09-14 - adopter-runtime-identity-guard-20260914: completed the guarded-root boundary — closed `guarded_file`/`adopter_root_runtime` policy enforced by `propose-lock` and `check`, destination-only runtime identity in migration and Sentinel, controls-only provenance, and 45-test coverage; all criteria verified.
- 2026-09-14 - adopter-runtime-identity-guard-20260914: grounded the pending guarded-root criterion in Phase 1 behavior — a closed `guarded_file` projection with the `adopter_root_runtime` destination policy, the `propose-lock`/`check` `policy_violation` fail-closed behavior, and regression coverage for the original reuse guide, renamed verbose lineage, and current minimal lineage escapes.
- 2026-09-14 - adopter-runtime-identity-guard-20260914: replaced the runtime-visible-provenance criterion with the pending controls-only provenance and guarded-root requirement after CodeRelay demonstrated that digest compliance could bless semantically wrong active-runtime content.
...
- 2026-09-14 - The adopter runtime identity boundary is machine-enforced: a closed `guarded_file` projection and the `adopter_root_runtime` destination policy make a prohibited or marker-less adopter root fail closed at both lock proposal and compliance check, so digest compliance can no longer bless a semantically wrong active runtime; AICore's own source root keeps its reuse guide, `.aicore` controls remain the provenance record, and existing adopters are non-compliant until synchronized after the AICore release.
- 2026-09-14 - Runtime-visible AICore provenance is superseded: adopter `AGENTS.md` files identify only their destination, while `.aicore` declaration/lock metadata remains the authoritative source-identity record; the engine and Sentinel enforce the boundary.
