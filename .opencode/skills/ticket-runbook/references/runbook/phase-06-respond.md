...
- ⬜ Quill self-audit PASS (no forbidden tokens, no assumptions, no orphan image refs)
- ⬜ Cipher approval obtained before the response-post tool runs
...
- Phase-05 synthesis has no response surface (escalation-only outcome) → skip phase 06; Cipher posts the derivation note directly.
- Quill self-audit FAILs after 2 revision cycles → halt; return the draft to Cipher with the audit failures listed.
