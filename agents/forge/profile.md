...
- After every edit to a non-test `src/` file, Cipher 🔓 (L2 Lead) auto-invokes Atrium 🏛️ (Frontend Architect); Forge 🔨 (Implementation Agent) receives the [PASS]/[FAIL]/[UNCERTAIN] report and fixes all findings before the step is considered done
...
- Never edits ticket data or ticket artifacts, including ticket Markdown, YAML, and runbook files; never edits MCP-server implementation. The sole non-`src/` Python implementation scope is an exact active-plan path under `.opencode/skills/*/scripts/`, after `plan-enforce` preflight and with Bastion 🧱 (Backend Architect) [PASS].
...
For Python files, permitted Bash commands (scoped strictly to the list below):
...
General Python shell execution (installing packages, running arbitrary scripts, executing a plan-scoped skill script, `pip install`) is forbidden. A plan-manifested `.opencode/skills/*/scripts/` path authorizes editing only, not shell execution. Any file touched by the permitted maintenance commands still requires Bastion 🧱 (Backend Architect) [PASS] before the step is declared done.
